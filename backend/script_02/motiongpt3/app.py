...
def _pick_server_port(preferred_port: int) -> int:
    if _is_port_available(preferred_port):
        return preferred_port
    # Ask OS for any free ephemeral port when preferred one is occupied.
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        s.bind(("localhost", 0))
...
def bot(history, motion_uploaded, data_stored, method, task='t2m'):
    ...
    if task in ['t2t', 'm2t']:
        ...
        np.savez(os.path.join(output_dir, "final_hidden_text.npz"), **save_dict)
        # fname = time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime(
        # time.time())) + str(np.random.randint(10000, 99999))
...
if __name__ == "__main__":
    with torch.no_grad():
        preferred_port = int(os.getenv("GRADIO_SERVER_PORT", "8888"))
        server_port = _pick_server_port(preferred_port)
        if server_port != preferred_port:
            print(
                f"Port {preferred_port} is busy. Starting Gradio on available port {server_port}."
            )
        demo.launch(server_name="localhost", server_port=server_port, debug=True, share=True)
