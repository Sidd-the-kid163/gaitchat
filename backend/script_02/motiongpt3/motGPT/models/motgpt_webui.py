...
#from motGPT.utils.render_utils ----- import render_motion not needed
...
class MotGPT(BaseModel):
    ...
    def forward(self, batch, task="t2m"):
        ...
        if task in ['t2m', 'pred', 'prediction', 'inbetween']:
            # add hook before generate_direct_motion
            #hook = self.lm.language_model.transformer.ln_f.register_forward_hook(hook_fn)
            #semantic_embeddings = hidden_cache['hidden'][self.lm.mod_id].detach().cpu()  # (bs, seq_len, 768)
            hidden_cache = {}
            def hook_fn(module, input, output):
                hidden_cache['hidden'] = output
            hook = self.lm.language_model.transformer.ln_f.register_forward_hook(hook_fn)
            print([n for n, _ in self.lm.language_model.named_modules() if 'ln_f' in n])
            outputs, source_attention_mask = self.lm.generate_direct_motion(
                    texts,
                    motion_tokens=motion_tokens_input,
                    )
            hook.remove()
            print('keys:', hidden_cache.keys())
            semantic_embeddings = hidden_cache['hidden'][self.lm.mod_id].detach().cpu()  # (bs, seq_len, 768)
            ...
            outputs = {
                ...
                "joints": joints_rst,
                "length": lengths,
                "semantic_embeddings_motion": semantic_embeddings
            }
        elif task in ['m2t', 't2t']:
            # add hook before generate_direct
            hidden_cache = {}
            def hook_fn(module, input, output):
                hidden_cache['hidden'] = output
            hook = self.lm.language_model.transformer.ln_f.register_forward_hook(hook_fn)
            ...
            outputs_tokens, cleaned_text, source_attention_mask = self.lm.generate_direct(
                texts,
                motion_tokens=motion_tokens_input,
                max_length=40,
                num_beams=1,
            ...
            outputs = {
                ...
                "joints": None,
                "length": None,
                "semantic_embeddings_text": semantic_embeddings
            }
        else:
        ...
        if task in ['t2m', 'pred', 'prediction', 'inbetween']:
            print("t2m branch")
            print("mod2id:", self.lm.mod2id)
            print("motion mod_id:", self.lm.mod_id)
            print("text mod_id:", self.lm.mod2id['text'])
      
            # generate_direct_motion should now return:
            # outputs, source_attention_mask, source_input_ids, all_hidden_states
            outputs, source_attention_mask, source_input_ids, all_hidden_states = self.lm.generate_direct_motion(
                texts,
                motion_tokens=motion_tokens_input,
            )
            ...
            # all_hidden_states[layer_idx][modality_idx] -> tensor [bs, seq_len, 768]
            semantic_embeddings = []
            semantic_positions = []

            for layer_idx in range(len(all_hidden_states)):
                hidden_motion = all_hidden_states[layer_idx][self.lm.mod_id].detach().cpu()
                ...
                for b in range(hidden_motion.shape[0]):
                    ...
                    som_idx = (ids_valid == self.lm.som_id).nonzero(as_tuple=True)[0]
                    eom_idx = (ids_valid == self.lm.eom_id).nonzero(as_tuple=True)[0]
            ...
            outputs = {
                ...
                "length": lengths,
                "semantic_embeddings_motion": semantic_embeddings,
                "semantic_positions_text": semantic_positions
            }
        elif task in ['m2t', 't2t']:
            outputs_tokens, cleaned_text, source_attention_mask, source_input_ids, all_hidden = self.lm.generate_direct(
                texts,
                motion_tokens=motion_tokens_input,
                max_length=40,
                num_beams=1,
            ...
            # all_hidden[layer_idx] = [bs, num_generated_tokens, 768]
            semantic_embeddings = []

            for layer_idx in range(len(all_hidden)):
                hidden_text = all_hidden[layer_idx].detach().cpu()

                # batch size 1 assumed; if batching later, handle per-sample similarly
                b = 0
                hidden_out = hidden_text[b]

                print(f"m2t layer {layer_idx} output shape: {tuple(hidden_out.shape)}")
