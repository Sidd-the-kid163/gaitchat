...
class MLM(nn.Module):
    ...
    def generate_direct(self,
        ...
        """ my structure print
        # extract hidden states after outputs and output_ids are defined
        print("type(outputs.hidden_states):", type(outputs.hidden_states))
        print("len(outputs.hidden_states):", len(outputs.hidden_states))
        print("type(outputs.hidden_states[0]):", type(outputs.hidden_states[0]))
        if isinstance(outputs.hidden_states[0], (list, tuple)):
            print("len(outputs.hidden_states[0]):", len(outputs.hidden_states[0]))
            for j, x in enumerate(outputs.hidden_states[0]):
                print(f"layer0 branch{j} shape:", x.shape)
        else:
            print("layer0 shape:", outputs.hidden_states[0].shape)
        """
        # In this custom generate path, hidden states are collected as modality-first
        # lists of per-step tensors; keep only the generated modality trajectory.
        all_hidden = []
        modality_hidden_steps = outputs.hidden_states[mid]
        if len(modality_hidden_steps) > 0:
            all_hidden.append(torch.cat(modality_hidden_steps, dim=1))  # [bs, num_generated_tokens, hidden_dim]
        #torch.set_printoptions(threshold=float('inf'), linewidth=1000)
        #print(all_hidden)
        output_is_motion = output_ids == self.motion_holder_id_out
        motion_latents = []

        return motion_latents, cleaned_text, source_attention_mask, source_input_ids, all_hidden
    ...
    def generate_direct_motion(self,
        ...
        if self.lm_type == 'encdec':
            ...
            #hidden = outputs.hidden_states[self.mod_id]  # [bs, labels_seq_len, emb_dim(768)]
            ...
            for layer_idx in range(len(hidden)):
                ...
                for b in range(hidden_motion.shape[0]):
                    attn_b = source_attention_mask[b].detach().cpu().bool()
                    ids_b = source_input_ids[b].detach().cpu()
                    ...
                    output_positions = full_positions[s+1:e]
                    hidden_out = hidden_motion[b][output_positions]

                    print(f"t2m layer {layer_idx} sample {b} shape after removing input/placeholders: {tuple(hidden_out.shape)}")

                    semantic_embeddings.append(hidden_out)
                    semantic_positions.append(output_positions)
        last_hidden = hidden[-1][self.mod_id].detach().cpu()   # [bs, seq_len, 768]
        #torch.set_printoptions(threshold=float('inf'), linewidth=1000) print entirely
        #print(last_hidden)
        # batch size 1 assumed
        b = 0

        saved_last_hidden = semantic_embeddings[-1]      # [num_output_tokens, 768]
        #print(saved_last_hidden)
        saved_last_pos = semantic_positions[-1]          # [num_output_tokens]

        # clone full last layer hidden
        last_hidden_reconstructed = last_hidden.clone()

        # write saved output back into the original positions
        last_hidden_reconstructed[b, saved_last_pos, :] = saved_last_hidden

        output_is_motion_cpu = output_is_motion.detach().cpu()

        # original latents from last_hidden
        orig_latents = torch.stack([
            h[output_is_motion_cpu[i], :]
            for i, h in enumerate(last_hidden)
        ]).detach().cpu()

        # reconstructed latents from overwritten last_hidden
        recon_latents = torch.stack([
            h[output_is_motion_cpu[i], :]
            for i, h in enumerate(last_hidden_reconstructed)
        ]).detach().cpu()
        """
        ...
        return latents, source_attention_mask, source_input_ids, hidden
