...
if __name__ == '__main__':
    ...
    for r in range(opt.repeat_times):
        ...
        for k, (caption, joint_data)  in enumerate(zip(captions, data)):
            ...
            save_path = pjoin(animation_path, "output.mp4")
            #ik_save_path = pjoin(animation_path, "sample%d_repeat%d_len%d_ik.mp4"%(k, r, m_length[k])) leave ik for now
            ...
            np.save(pjoin(joint_path, "output.npy"), joint)
            #np.save(pjoin(joint_path, "sample%d_repeat%d_len%d_ik.npy"%(k, r, m_length[k])), ik_joint) leave ik for now
            for fname in os.listdir(animation_path):
                fpath = os.path.abspath(os.path.join(animation_path, fname))
                if os.path.isfile(fpath) and fname != "output.mp4":
