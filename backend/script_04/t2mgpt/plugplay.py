...
for line in sys.stdin:
    ...
    for part in line.split("|"):
        ...
        if part.startswith("TEXT:"):
            ...
            try:
                score_value = int(txt)
                db.score(score_value, 4)  # setnum is the chatbox number
    ...
    subprocess.run([
    "python", "demo.py", "--text", f"{txt}"
    ], check=True)
    #print(data_stored[-1])
    if os.path.exists("./cache/example.gif"):
        timestamp = int(time.time())
        print(f"FILE:gif:{os.path.abspath('./cache/example.gif')}?t={timestamp}", flush=True)
        db.add_history(
            txt,
