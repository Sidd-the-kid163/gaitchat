...
def plot_3d_motion(args, figsize=(10, 10), fps=120, radius=4):
    ...
    # Global bounds across ALL frames
    global_min = data.min(axis=(0, 1))
    global_max = data.max(axis=(0, 1))

    # Center (optional but recommended)
    center = (global_min + global_max) / 2
    range_max = (global_max - global_min).max() / 2

    def update(index):

        def init():
            ax.set_xlim(center[0] - range_max, center[0] + range_max)
            ax.set_ylim(0, center[1] + range_max)   # keep floor at 0
            ax.set_zlim(center[2] - range_max, center[2] + range_max)
            ax.grid(False)
