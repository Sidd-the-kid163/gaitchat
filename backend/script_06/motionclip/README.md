# MotionCLIP
    during inference, for editing and interpolation, mapping was a problem so a way around was found by doing mapping off on the side and model separately in a computationally low way
    python -m pip install git+https://github.com/openai/CLIP.git
    python -m pip install "human-body-prior<2.0"
    make sure to run these two commands after setting env

  def get_datasets(parameters, clip_preprocess, split="train"):
    # Return dummy datasets for inference — dataset not needed when using pretrained model
    ...
    modify to this in get_dataset.py if running inference
